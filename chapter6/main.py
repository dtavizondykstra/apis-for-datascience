"""Fast API application for chapter 4."""
...
def read_player(
    player_id: int = Path(..., description="The unique identifier for the player in SWC"),
    db: Session = Depends(get_db)
):
...
@app.get(
    ...
    description="""Use this endpoint to get lists of SWC fantasy football leagues. You can use the skip and limit to perform pagination of the API. League name is not guaranteed to be unique. Don't use the League ID for counting or logic, because that is an internal ID and is not guaranteed to be sequential""",
...
def read_league(
    league_id: int = Path(..., description="The unique identifier for the league in SWC"),
    db: Session = Depends(get_db)
):
...
def read_teams(
    ...
    limit: int = Query(100, description="The number of records to return after the skipped records."),
    minimum_last_changed_date: date = Query(None, description="The minimum data of change that you want to return records. Exclude any records changed before this."),
    team_id: int = Query(None, description="The unique identifier for the team in SWC"),
